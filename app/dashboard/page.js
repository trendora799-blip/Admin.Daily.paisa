...
export default function DashboardPage() {
  ...
  const [stats, setStats] = useState({ totalUsers: 0, totalRevenue: 0, pendingDeposits: 0, totalWithdrawals: 0, revenueChart: [], usersChart: [] });
  ...
  const fetchData = async () => {
    try {
      const [usersRes, txRes, wdRes] = await Promise.all([
        fetch('/api/users'), fetch('/api/transactions'), fetch('/api/withdrawals')
      ]);
      if (!usersRes.ok || !txRes.ok || !wdRes.ok) throw new Error('Failed to load data');
      ...
      const days = [], revenueData = [], usersData = [];
      for (let i = 6; i >= 0; i--) {
        const d = new Date(); d.setDate(d.getDate() - i);
        const dateStr = d.toISOString().split('T')[0];
        const dayName = d.toLocaleDateString('en-US', { weekday: 'short' });
        days.push(dayName);
        revenueData.push(txs.filter(t => t.status === 'approved' && t.type === 'deposit' && t.createdAt?.startsWith(dateStr)).reduce((sum, t) => sum + (t.amount || 0), 0));
        usersData.push(regularUsers.filter(u => u.createdAt?.startsWith(dateStr)).length);
      }
      const maxRev = Math.max(...revenueData, 1);
      const maxUsers = Math.max(...usersData, 1);

      setStats({ totalUsers: regularUsers.length, totalRevenue, pendingDeposits, totalWithdrawals, revenueChart: days.map((day, i) => ({ day, value: revenueData[i], max: maxRev })), usersChart: days.map((day, i) => ({ day, value: usersData[i], max: maxUsers })) });
    ...
    <div className="space-y-6 animate-fade-in p-4 pb-24">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-slate-800 rounded-xl border border-slate-700">
          ...
          <p className="text-2xl font-bold text-red-400 mt-1">₹{stats.totalWithdrawals}</p>
        </div>
      </div>
          ...
          <div className="flex items-end justify-between h-48 gap-2">
            {stats.revenueChart.map((item, i) => (
              <div key={i} className="flex flex-col items-center flex-1 h-full justify-end">
                ...
                <div className="w-full max-w-[30px] bg-gradient-to-t from-emerald-600 to-emerald-400 rounded-t-md" style={{ height: `${(item.value / item.max) * 100}%`, minHeight: item.value > 0 ? '4px' : '0' }}></div>
        ...
        <div className="p-6 bg-slate-800 rounded-xl border border-slate-700">
          ...
          <div className="flex items-end justify-between h-48 gap-2">
            {stats.usersChart.map((item, i) => (
              <div key={i} className="flex flex-col items-center flex-1 h-full justify-end">
                <span className="text-[10px] text-slate-400 mb-1">{item.value}</span>
                <div className="w-full max-w-[30px] bg-gradient-to-t from-brand-600 to-brand-400 rounded-t-md" style={{ height: `${(item.value / item.max) * 100}%`, minHeight: item.value > 0 ? '4px' : '0' }}></div>
                <span className="text-[10px] text-slate-500 mt-2">{item.day}</span>
              </div>
            ))}
          </div>
